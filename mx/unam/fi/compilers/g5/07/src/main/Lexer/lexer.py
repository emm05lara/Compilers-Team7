...
lexemes extracted from a source code that has been written in C language.
...
class Lexer:
    ...
    def __init__(self, lexemes, resource_dir: str):
        """
        Set up the lexer by storing the input, preparing the token categories,
        and loading the required resource files.
        """
        ...
        # Dictionary used to group tokens by category
        ...
        # Initialize the token categories used 
    ...
    def _load_keywords(self):
        """
        Read our file keywords.txt and store all the words in the keywords set.
        Words in this set are later used to distinguish keywords from identifiers.
        """
    ...
    def _load_tokens(self):
        """
        Read tokens.txt and register any new token.
        By this the lexer to support some extra categories beyond the default ones.
        """
    ...
    def reset(self):
        """
        Clear all stored tokens and counters.
        """
    ...
    def tokenize(self):
        """
        Run the full lexical analysis on the stored lexemes.
 
        The method  removes comments from the source text, and then it scans
        each line using regular expressions to identify and classify tokens.
        
        Every token is recorded with its type, value, line, and column.
        Unrecognized sequences are separately as Unknown tokens.
 
        Returns a list of dictionaries, one per token found.
        """
        self.reset()
        ...
        # Combined pattern that tries each category in order; longer patterns come first
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            for match in matches:
                ...
                # Determine the category of each token based on its pattern
                if token == "#":
            ...
            # Process remaining unknown characters 
    ...
    def _handle_unknowns(self, line, consumed_ranges, line_num):
        """
        Find the remaining unknown characters and group them into Unknown tokens.
        """
        mask = [True] * len(line)
    ...
    def _add_unknown_to_list(self, value, line_num, line):
        """
        Register an unrecognized token in the Unknown category and add it to the tokens list.
        """
